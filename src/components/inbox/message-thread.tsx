    ...
    <div className={cn("flex min-w-0 flex-1 flex-col", DOODLE_BG_CLASSES)}>
      ...
      <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-3 py-3 sm:px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Assign dropdown — admins can reassign; agents see static assignee */}
          {canSeeAll ? (
            <DropdownMenu>
              <DropdownMenuTrigger
                className={cn(
            ...
            <span
              className={cn(
                "inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md bg-muted/60",
                assignedAgentId ? "text-primary font-medium" : "text-muted-foreground"
              )}
            >
              <UserPlus className="h-3 w-3" />
              <span className="hidden sm:inline">{assignLabel}</span>
